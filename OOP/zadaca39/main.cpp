...
class NoCourseExeption {
private:
    int indeks;
public:
    NoCourseExeption(int indeks) {
        this->indeks = indeks;
    }
...
public:
    ...
    int getBodovi() {
        if (n == 0) {
            throw NoCourseExeption(indeks);
        }
        int plus;
        plus = (20 * brNaCasoviNedelno) / n;
        return Student::getBodovi() + plus;
    }
    ...
    const char *getKurs(int i) {
        return lista[i].getIme();
    }
};
...
Student &vratiNajdobroRangiran(Student **studenti, int n) {
    ...
    for (int i = 0; i < n; i++) {
        try {
            tmp = studenti[i]->getBodovi();
            if (tmp > max) {
                max = tmp;
                index = i;
            }
        }
        catch (NoCourseExeption e) {
            e.showMessage();
        }
    }
