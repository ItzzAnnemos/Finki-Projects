...
public class SLLTree <E> implements Tree<E>{
    ...
    public SLLNode<E> findNode(SLLNode<E> currentNode, E targetElement, SLLTree<E> tree) {
        if (currentNode == null) {
            return null;
        }

        if (currentNode.getElement().equals(targetElement)) {
            return currentNode;
        }

        SLLNode<E> tmp = currentNode.firstChild;
        while (tmp != null) {
            SLLNode<E> result = findNode(tmp, targetElement, tree);
            if (result != null) {
                return result;
            }
            tmp = tmp.sibling;
